...
public abstract class HbaseAbstractTask {
    ...
    public HbaseAbstractTask(com.alibaba.datax.common.util.Configuration configuration) {
        ...
        this.htable = Hbase132Helper.getTable(configuration);

        this.encoding = configuration.getString(Key.ENCODING,Constant.DEFAULT_ENCODING);
        this.startKey = Hbase132Helper.convertInnerStartRowkey(configuration);
        this.endKey =  Hbase132Helper.convertInnerEndRowkey(configuration);
    ...
    public void close()  {
        Hbase132Helper.closeResultScanner(this.resultScanner);
        Hbase132Helper.closeTable(this.htable);
    }
    ...
    public Column convertBytesToAssignType(ColumnType columnType, byte[] byteArray, String dateformat) throws Exception {
        ...
        switch (columnType) {
            ...
            case DATE:
                ...
                break;
            default:
                throw DataXException.asDataXException(Hbase132ReaderErrorCode.ILLEGAL_VALUE, "Hbasereader 不支持您配置的列类型:" + columnType);
        }
        return column;
    }
    ...
    public Column convertValueToAssignType(ColumnType columnType, String constantValue, String dateformat) throws Exception {
        ...
        switch (columnType) {
            ...
            case DATE:
                ...
                break;
            default:
                throw DataXException.asDataXException(Hbase132ReaderErrorCode.ILLEGAL_VALUE, "Hbasereader 常量列不支持您配置的列类型:" + columnType);
        }
        return column;
    }
}
