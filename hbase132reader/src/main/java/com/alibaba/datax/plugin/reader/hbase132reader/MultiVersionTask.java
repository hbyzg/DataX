...
public abstract class MultiVersionTask extends HbaseAbstractTask {
    ...
    public MultiVersionTask(Configuration configuration) {
        ...
        this.familyQualifierMap = Hbase132Helper.parseColumnOfMultiversionMode(this.column);

        try {
            MultiVersionTask.COLON_BYTE = ":".getBytes("utf8");
        } catch (UnsupportedEncodingException e) {
            throw DataXException.asDataXException(Hbase132ReaderErrorCode.PREPAR_READ_ERROR, "系统内部获取 列族与列名冒号分隔符的二进制时失败.", e);
        }
    }
